export default class MovieService {
  _apiKey = 'api_key=5f8af770dea0fa6a65a747f094d119f6'
  ...
  async getResource(url, page = 1) {
    const res = await fetch(`${this._apiBaseUrl}${url}?${this._apiKey}&page=${page}`)
    if (!res.ok) {
      throw new Error(`Couldnt fetch${url} received ${res.status}`)
    }
  ...
  async getResourceBySearch(request, page = 1) {
    const res = await fetch(`${this._apiBaseUrl}search/movie?query=${request}&${this._apiKey}&page=${page}`)
    if (!res.ok) {
